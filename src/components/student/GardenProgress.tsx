...
export default function GardenProgress({
  ...
  const foodCount = Math.min(10, Math.max(2, practiceTimes + quickTests));
  const treeStage = completedLessons >= 6 ? "🌳" : completedLessons >= 3 ? "🌿" : "🌱";
      ...
      <div className="grid gap-8 lg:grid-cols-[1.25fr_0.75fr] lg:items-center">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
            ...
            khu vườn càng xanh, Bu càng có thêm thức ăn học tập.
          </p>

          <div className="mt-6 grid gap-3 sm:grid-cols-4">
            <div className="rounded-2xl bg-white/85 p-4 shadow-sm">
              <p className="text-xs text-slate-500">Thời gian học</p>
              <p className="mt-1 text-2xl font-bold text-slate-800">{studyMinutes}p</p>
            </div>

            <div className="rounded-2xl bg-white/85 p-4 shadow-sm">
              <p className="text-xs text-slate-500">Bài đạt</p>
              <p className="mt-1 text-2xl font-bold text-slate-800">{completedLessons}</p>
            </div>

            <div className="rounded-2xl bg-white/85 p-4 shadow-sm">
              <p className="text-xs text-slate-500">Luyện tập</p>
              <p className="mt-1 text-2xl font-bold text-slate-800">{practiceTimes}</p>
            </div>

            <div className="rounded-2xl bg-white/85 p-4 shadow-sm">
              <p className="text-xs text-slate-500">Quick-test</p>
              <p className="mt-1 text-2xl font-bold text-slate-800">{quickTests}</p>
            </div>
          </div>
        </div>

        <div className="relative min-h-[300px] overflow-hidden rounded-[36px] bg-white/70 p-6 shadow-sm">
          <div className="absolute left-8 top-8 text-6xl">{treeStage}</div>

          <div className="absolute right-8 top-6 h-36 w-36">
            <Image
              src="/bu-mascot.png"
              alt="Bu"
              fill
          ...
          <div className="absolute bottom-24 left-8 right-8 flex flex-wrap gap-2">
          ...
          <div className="absolute bottom-10 left-8 right-8 flex flex-wrap gap-2">
          ...
          <div className="absolute bottom-4 left-6 right-6 rounded-2xl bg-emerald-600 px-4 py-3 text-center text-sm font-semibold text-white">
            {meta.label} đang lớn lên nhờ dữ liệu học tập của em
          </div>
        </div>
      </div>
    </section>
